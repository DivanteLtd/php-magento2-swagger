...
class BundleProductOptionRepositoryV1Api
{
     ...
     * @param int $option_id  (required)
     * @throws \Swagger\Magento2Client\ApiException on non-2xx response
     * @return array of \Swagger\Magento2Client\Model\BundleDataOptionInterface, HTTP status code, HTTP response headers (array of strings)
     */
    public function bundleProductOptionRepositoryV1GetGetWithHttpInfo($sku, $option_id)
    {
        ...
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                ...
                $httpBody,
                $headerParams,
                '\Swagger\Magento2Client\Model\BundleDataOptionInterface',
                '/V1/bundle-products/{sku}/options/{optionId}'
            );

            return [$this->apiClient->getSerializer()->deserialize($response, '\Swagger\Magento2Client\Model\BundleDataOptionInterface', $httpHeader), $statusCode, $httpHeader];
        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\Swagger\Magento2Client\Model\BundleDataOptionInterface', $e->getResponseHeaders());
                    $e->setResponseObject($data);
                    break;
                case 400:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\Swagger\Magento2Client\Model\ErrorResponse', $e->getResponseHeaders());
